...
export function SpecimenDetailPage() {
  ...
  const morphColor = SPECIMEN_CATEGORY_COLORS.morphology;
  const habColor = SPECIMEN_CATEGORY_COLORS.habitat;
  const discColor = SPECIMEN_CATEGORY_COLORS.discovery;
  const metaColor = SPECIMEN_CATEGORY_COLORS.metabolism;
  const appColor = SPECIMEN_CATEGORY_COLORS.application;

  const tempML = (specimen.habitat.temperature[0] / 120 * 100);
  const tempW = ((specimen.habitat.temperature[1] - specimen.habitat.temperature[0]) / 120 * 100);
  const phML = (specimen.habitat.phRange[0] / 14 * 100);
  const phW = ((specimen.habitat.phRange[1] - specimen.habitat.phRange[0]) / 14 * 100);
  ...
  return (
    <div className="relative min-h-screen pt-32 pb-20">
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse 60% 50% at 30% 10%, ' + colorAlpha(color, '22') + ', transparent), radial-gradient(ellipse 50% 40% at 80% 60%, ' + colorAlpha(color, '15') + ', transparent)',
        }}
      />
      ...
      <div className="container mx-auto px-6 relative z-10">
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:items-end lg:justify-between mb-8">
            <div className="flex items-start gap-5">
              <div
                ...
                style={{
                  background: 'linear-gradient(135deg, ' + colorAlpha(color, '88') + ', transparent 60%)',
                }}
              >
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <span
                    ...
                    style={{
                      background: colorAlpha(color, '15'),
                      color,
                      border: '1px solid ' + colorAlpha(color, '30'),
                    }}
                  >
                ...
                <h1 className="font-display text-5xl md:text-6xl font-bold text-text-light mb-2 leading-tight" style={{ textShadow: '0 0 40px ' + colorAlpha(color, '40') }}>
                  {commonName}
                </h1>
            ...
            {sections.map((section) => {
              ...
              return (
                <button
                  ...
                  className={'px-5 py-2.5 rounded-full font-mono text-sm transition-all duration-300 flex items-center gap-2 ' + (isActive ? 'shadow-glow-sm' : '')}
                  style={{
                    background: isActive ? colorAlpha(section.color, '15') : 'rgba(255,255,255,0.03)',
                    color: isActive ? section.color : '#94a3b8',
                    border: '1px solid ' + (isActive ? colorAlpha(section.color, '50') : 'rgba(255,255,255,0.1)'),
                  }}
                >
                  ...
                  <div className="space-y-2">
                    {Object.entries(specimen.taxonomicClassification).map(([rank, name], idx) => {
                      const rankLabels: Record<string, string> = {
                        kingdom: '界', phylum: '门', class: '纲', order: '目',
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="font-mono text-xs text-text-light">{name}</span>
                          {idx < 6 && <ChevronRight className="w-3 h-3 text-text-muted/40 ml-auto" />}
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {specimen.references.map((ref, idx) => (
                    ...
                    >
                      ...
                      <p className="font-mono text-[11px] text-text-muted/70">{ref.authors}</p>
              ...
              <div className="lg:col-span-3">
                <div className="glass-card p-6">
                  <div className="flex items-center gap-3 mb-6">
                    <Brain className="w-5 h-5" style={{ color: morphColor }} />
                    <h2 className="font-display text-xl font-semibold text-text-light">形态学结构</h2>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                    {[
                      ...
                      <div key={item.label} className="p-3 rounded-xl bg-background/30 border border-white/5">
                        <div className="font-mono text-[10px] text-text-muted tracking-wide mb-1">{item.label}</div>
                        <div className="font-display text-base font-semibold text-text-light">{item.value}</div>
                      </div>
                    ))}
                  </div>

                  <div className="grid grid-cols-3 gap-3 mb-6">
                    <div className={'p-3 rounded-xl border ' + (specimen.morphology.motility ? 'bg-green-400/10 border-green-400/20 text-green-400' : 'bg-white/5 border-white/10 text-text-muted/50')}>
                      <div className="font-mono text-[10px] tracking-wider">运动能力</div>
                      <div className="font-display text-sm font-semibold">{specimen.morphology.motility ? '有运动性' : '无运动'}</div>
                    </div>
                    <div className={'p-3 rounded-xl border ' + (specimen.morphology.sporeFormation ? 'bg-glow-gold/10 border-glow-gold/20 text-glow-gold' : 'bg-white/5 border-white/10 text-text-muted/50')}>
                      <div className="font-mono text-[10px] tracking-wider">产孢子</div>
                      <div className="font-display text-sm font-semibold">{specimen.morphology.sporeFormation ? '可形成孢子' : '不产孢子'}</div>
                    </div>
                    <div className={'p-3 rounded-xl border ' + (specimen.morphology.capsule ? 'bg-glow-primary/10 border-glow-primary/20 text-glow-primary' : 'bg-white/5 border-white/10 text-text-muted/50')}>
                      <div className="font-mono text-[10px] tracking-wider">荚膜形成</div>
                      <div className="font-display text-sm font-semibold">{specimen.morphology.capsule ? '有荚膜' : '无荚膜'}</div>
                    </div>
                  </div>

                  <h3 className="font-display text-base font-semibold text-text-light mb-3">特殊结构特征</h3>
                  <div className="flex flex-wrap gap-2 mb-6">
                    {specimen.morphology.specialFeatures.map((feature) => (
                      <span
                        ...
                        style={{
                          background: colorAlpha(morphColor, '10'),
                          color: morphColor,
                          borderColor: colorAlpha(morphColor, '30'),
                        }}
                      >
                        {feature}
                      </span>
                  ...
                  <h3 className="font-display text-base font-semibold text-text-light mb-3">详细形态特征描述</h3>
                  <div className="space-y-3">
                    {specimen.morphology.detailedFeatures.map((feat) => (
                      <div
                        ...
                        style={{ borderLeftColor: morphColor }}
                      >
                        <div className="flex items-start justify-between mb-1">
                          <h4 className="font-display text-sm font-semibold text-text-light">{feat.label}</h4>
                          <span
                            className="font-mono text-xs px-2 py-0.5 rounded"
                            style={{ background: colorAlpha(morphColor, '12'), color: morphColor }}
                          >
                            {feat.value}
                          </span>
                        </div>
                        <p className="font-mono text-xs text-text-muted/80">{feat.description}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="space-y-8 animate-fade-in-up stagger-3 opacity-0">
            <div className="grid lg:grid-cols-5 gap-8">
              <div className="lg:col-span-3">
                <div className="glass-card p-6">
                  <div className="flex items-center gap-3 mb-6">
                    <Leaf className="w-5 h-5" style={{ color: habColor }} />
                    <h2 className="font-display text-xl font-semibold text-text-light">生存环境生态</h2>
                  </div>

                  <div className="p-4 rounded-2xl border mb-6"
                    style={{ background: colorAlpha(habColor, '08'), borderColor: colorAlpha(habColor, '20') }}
                  >
                    <div className="font-mono text-[10px] text-text-muted tracking-widest uppercase mb-2" style={{ color: habColor }}>
                      栖息地类型：{specimen.habitat.type}
                    </div>
                    <p className="font-mono text-sm text-text-muted/80 leading-relaxed">{specimen.habitat.description}</p>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-6">
                    <div className="p-4 rounded-xl bg-background/30 border border-white/5">
                      ...
                      <div className="font-display text-lg font-bold text-text-light">{specimen.habitat.temperature[0]}°C</div>
                      <div className="w-full h-1.5 rounded-full bg-white/10 mt-2 overflow-hidden">
                        <div
                          ...
                          style={{ marginLeft: pct(tempML), width: pct(tempW) }}
                        />
                      </div>
                    ...
                    <div className="p-4 rounded-xl bg-background/30 border border-white/5">
                      ...
                      <div className="w-full h-1.5 rounded-full bg-white/10 mt-2 overflow-hidden">
                        <div
                          ...
                          style={{ marginLeft: pct(phML), width: pct(phW) }}
                        />
                      </div>
                    </div>
                    <div className="p-4 rounded-xl bg-background/30 border border-white/5">
                      ...
                      <div className="font-mono text-[10px] text-text-muted mt-2">盐度: {specimen.habitat.salinity}</div>
                    </div>
                  </div>

                  <h3 className="font-display text-base font-semibold text-text-light mb-3">常见分布地点</h3>
                  <div className="grid grid-cols-2 gap-2 mb-6">
                    {specimen.habitat.commonLocations.map((loc) => (
                      <div key={loc} className="flex items-center gap-2 p-3 rounded-xl bg-background/30 border border-white/5">
                        <Globe2 className="w-3.5 h-3.5" style={{ color: habColor }} />
                        <span className="font-mono text-xs text-text-light">{loc}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="p-4 rounded-2xl border border-green-400/15 bg-green-400/5">
                    ...
                    <p className="font-mono text-sm text-text-muted/80 leading-relaxed">{specimen.habitat.ecologicalRole}</p>
                  </div>
                </div>
              </div>
              <div className="lg:col-span-2">
          ...
          <div className="space-y-8 animate-fade-in-up stagger-3 opacity-0">
            <div className="glass-card p-6">
              <div className="flex items-center gap-3 mb-8">
                <Clock className="w-5 h-5" style={{ color: discColor }} />
                <h2 className="font-display text-xl font-semibold text-text-light">发现历史时间线</h2>
              </div>
              <div className="relative">
                <div
                  className="absolute left-6 top-0 bottom-0 w-0.5"
                  style={{ background: 'linear-gradient(180deg, ' + colorAlpha(discColor, '66') + ', transparent)' }}
                />
                  ...
                  {specimen.discoveryTimeline.map((event, idx) => (
                    <div key={event.id} className="relative flex gap-6 pl-12">
                      <div
                        ...
                        style={{ background: colorAlpha(discColor, '20'), border: '2px solid ' + discColor }}
                      >
                        <span className="font-display text-lg font-bold" style={{ color: discColor }}>
                      ...
                      <div className="flex-1 p-5 rounded-2xl bg-background/40 border border-white/5 hover:border-glow-gold/30 transition-colors">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <div className="font-display text-3xl font-bold" style={{ color: discColor }}>{event.year}</div>
                            <h3 className="font-display text-lg font-semibold text-text-light mt-1">{event.title}</h3>
                          </div>
                        ...
                        <p className="font-mono text-sm text-text-muted/80 leading-relaxed mb-3">{event.description}</p>
                        <div className="p-3 rounded-xl" style={{ background: colorAlpha(discColor, '08'), borderLeft: '3px solid ' + discColor }}>
                          <div className="flex items-start gap-2">
                            <Star className="w-3.5 h-3.5 shrink-0 mt-0.5" style={{ color: discColor }} />
                            <p className="font-mono text-xs text-text-light/90 leading-relaxed">
                              <span className="font-semibold">科学意义：</span>{event.significance}
                            </p>
                          </div>
                        </div>
                      </div>
        ...
        {activeSection === 'metabolism' && (
          <div className="space-y-8 animate-fade-in-up stagger-3 opacity-0">
            <div className="glass-card p-6">
              <div className="flex items-center gap-3 mb-8">
                <Zap className="w-5 h-5" style={{ color: metaColor }} />
                <h2 className="font-display text-xl font-semibold text-text-light">代谢能力图谱</h2>
              </div>
              <div className="space-y-5">
                {specimen.metabolicCapabilities.map((cap) => (
                  <div key={cap.id} className="p-5 rounded-2xl bg-background/30 border border-white/5 hover:border-white/10 transition-all group">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-start gap-4">
                        <div
                          className="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0"
                          style={{ background: colorAlpha(metaColor, '15'), border: '1px solid ' + colorAlpha(metaColor, '30') }}
                        >
                          <TrendingUp className="w-7 h-7" style={{ color: metaColor }} />
                        </div>
                        <div>
                          <span className="font-mono text-[10px] text-text-muted tracking-widest uppercase">
                            {METABOLISM_TYPE_LABELS[cap.category]}
                          </span>
                          <h3 className="font-display text-lg font-semibold text-text-light">{cap.name}</h3>
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="font-mono text-[10px] text-text-muted tracking-wider mb-1">代谢效率</div>
                        <div className="flex items-center gap-2">
                          <div className="w-24 h-2 rounded-full bg-white/10 overflow-hidden">
                            <div
                              className="h-full rounded-full"
                              style={{
                                width: pct(cap.efficiency),
                                background: 'linear-gradient(90deg, ' + metaColor + ', ' + colorAlpha(metaColor, '88') + ')',
                              }}
                            />
                          </div>
                          <span className="font-display text-lg font-bold" style={{ color: metaColor }}>{cap.efficiency}%</span>
                        </div>
                      </div>
                    </div>
                    <p className="font-mono text-sm text-text-muted/80 leading-relaxed mb-4">{cap.description}</p>
                    <div className="grid md:grid-cols-3 gap-3">
                      <div className="p-3 rounded-xl bg-background/40 border border-white/5">
                        <div className="font-mono text-[10px] text-text-muted mb-1">反应底物</div>
                        <div className="flex flex-wrap gap-1">
                          {cap.substrates.map((s) => (
                            <span key={s} className="px-2 py-0.5 rounded text-[10px] font-mono bg-glow-red/10 text-glow-red/90 border border-glow-red/20">{s}</span>
                          ))}
                        </div>
                      </div>
                      <div className="p-3 rounded-xl bg-background/40 border border-white/5">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {cap.products.map((p) => (
                            <span key={p} className="px-2 py-0.5 rounded text-[10px] font-mono bg-green-400/10 text-green-400/90 border border-green-400/20">{p}</span>
                          ))}
                        </div>
                      </div>
                      <div className="p-3 rounded-xl bg-background/40 border border-white/5">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {cap.keyEnzymes.slice(0, 2).map((e) => (
                            <span key={e} className="px-2 py-0.5 rounded text-[10px] font-mono bg-glow-purple/10 text-glow-purple/90 border border-glow-purple/20">{e}</span>
                          ))}
                          {cap.keyEnzymes.length > 2 && (
                            <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-white/5 text-text-muted border border-white/10">+{cap.keyEnzymes.length - 2}</span>
                          )}
                        </div>
                      </div>
                    </div>
        ...
        {activeSection === 'application' && (
          <div className="space-y-8 animate-fade-in-up stagger-3 opacity-0">
            <div className="glass-card p-6">
              <div className="flex items-center gap-3 mb-8">
                <Factory className="w-5 h-5" style={{ color: appColor }} />
                <h2 className="font-display text-xl font-semibold text-text-light">工业与科研应用</h2>
              </div>
              <div className="space-y-6">
                {specimen.applications.map((app) => {
                  ...
                  return (
                    <div key={app.id} className="p-6 rounded-2xl bg-background/30 border border-white/5 overflow-hidden relative group">
                      <div className="absolute top-0 left-0 w-full h-1"
                        style={{ background: 'linear-gradient(90deg, ' + colorAlpha(catColor, '88') + ', ' + colorAlpha(catColor, '22') + ', transparent)' }}
                      />
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-5">
                        <div className="flex items-start gap-4">
                          <div
                            className="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0"
                            style={{ background: colorAlpha(catColor, '15'), border: '1px solid ' + colorAlpha(catColor, '30') }}
                          >
                          ...
                          <div>
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                              <span
                                ...
                                style={{ background: colorAlpha(catColor, '12'), color: catColor, border: '1px solid ' + colorAlpha(catColor, '30') }}
                              >
                                {INDUSTRIAL_CATEGORY_LABELS[app.category]}
                              </span>
                              <span
                                className="font-mono text-[10px] tracking-widest uppercase px-2 py-0.5 rounded-full"
                                style={{ background: colorAlpha(matColor, '12'), color: matColor, border: '1px solid ' + colorAlpha(matColor, '30') }}
                              >
                                {MATURITY_LABELS[app.maturity]}
                              </span>
                            </div>
                            <h3 className="font-display text-xl font-semibold text-text-light">{app.title}</h3>
                          </div>
                        </div>
                        <div className="mt-3 md:mt-0 md:text-right shrink-0">
                          ...
                          <div className="font-display text-base font-semibold" style={{ color: catColor }}>{app.marketScale}</div>
                        </div>
                      </div>

                      <p className="font-mono text-sm text-text-muted/80 leading-relaxed mb-4">{app.description}</p>

                      <div className="mb-4">
                        <div className="font-mono text-[10px] text-text-muted tracking-wider uppercase mb-2">核心优势</div>
                        <div className="grid md:grid-cols-2 gap-2">
                          {app.keyAdvantages.map((adv) => (
                            <div
                              key={adv}
                              className="flex items-start gap-2 p-2.5 rounded-xl"
                              style={{ background: colorAlpha(catColor, '08'), border: '1px solid ' + colorAlpha(catColor, '15') }}
                            >
                      ...
                      <div className="pt-4 border-t border-white/5">
                        <div className="font-mono text-[10px] text-text-muted tracking-wider uppercase mb-2">典型案例研究</div>
                          ...
                          {app.caseStudies.map((cs, idx) => (
                            <div key={idx} className="p-3 rounded-xl bg-background/40 border border-white/5">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <h4 className="font-display text-sm font-semibold text-text-light">{cs.title}</h4>
                              </div>
                              <div className="font-mono text-[10px]" style={{ color: catColor }}>{cs.organization}</div>
                              <p className="font-mono text-[11px] text-text-muted/80 mt-1 leading-relaxed">{cs.outcome}</p>
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        {activeSection === 'knowledge' && (
          <div className="animate-fade-in-up stagger-3 opacity-0">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h2 className="font-display text-xl font-semibold text-text-light">精选知识卡片集</h2>
